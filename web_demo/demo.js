...
function normalize() {
  ...
  output.textContent = data.text || t('(empty input)', '(entrada vacía)');
  details.innerHTML = `<div class="metric"><strong>${source.value.length}</strong><small>${t('Input characters','Caracteres de entrada')}</small></div><div class="metric"><strong>${data.replacements.length}</strong><small>${t('Mapped replacements','Reemplazos mapeados')}</small></div><div class="metric"><strong>${new Set(data.replacements.map((item) => item.from.toLowerCase())).size}</strong><small>${t('Unique expressions','Expresiones únicas')}</small></div>`;
  replacementList.innerHTML = data.replacements.map((item) => `<span class="badge">${escapeHtml(item.from)} → ${escapeHtml(item.to)}</span>`).join('') || `<span class="muted">${t('No configured slang was found; the input stays unchanged.','No se encontró jerga configurada; la entrada queda igual.')}</span>`;
}
...
init().catch(() => {});
document.addEventListener('mt:language', (event) => { language = event.detail.language; normalize(); });
